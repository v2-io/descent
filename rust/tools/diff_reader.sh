#!/bin/sh
# Reader differential: udon-core front-end tokens vs oracle lexer tokens on
# all fixture grammars. Per-fixture mismatch counts; details on demand with
...
for desc in rust/tests/fixtures/*.desc; do
  ...
  o=$("$ORACLE" tokens "$desc" 2>/dev/null) || { echo "ERR  $base (oracle)"; fail=1; continue; }
