...
//!
//! Deliberate v1 limits: no `--trace` plumbing; events are the owned
//! `StreamEvent` shape (streaming consumers own their bytes; the recursive
//! backend remains the zero-copy single-shot fast path). The generated
//! module imports `ParseErrorCode` / `StreamEvent` / `ParseResult` from the
//! sibling recursive module so the two backends interoperate.
    ...
    fn render_emit(&mut self, b: &mut String, cmd: &Command, ind: usize) {
        match cmd.ctype.as_str() {
            ...
            "inline_emit_bare" => {
                ...
                let _ = writeln!(
                    b,
                    "{:ind$}on_event(StreamEvent::{t} {{ content: Vec::new(), span: self.gspan() }});",
                    ""
                );
            }
            "inline_emit_mark" => {
            ...
            "inline_emit_literal" => {
                ...
                let _ = writeln!(
                    b,
                    "{:ind$}on_event(StreamEvent::{t} {{ content: b\"{}\".to_vec(), span: self.gspan() }});",
                    "",
                    esc_str(lit)
                );
            }
