...
Any descent change that alters generated code must be validated against the
primary consumer (UDON, `core/` in the umbrella repo) with an **immediate
before/after benchmark pair**: `cargo bench -p udon-core` at the prior pin,
then again after regenerating with the change — unconfounded by grammar
edits. The consumer's fixture gate proves correctness; the benchmark pair
proves performance. Byte-identical output is never a goal.
