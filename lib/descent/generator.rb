...
module Descent
  ...
  module LiquidFilters
    # Canonical byte values for DSL escape sequences.
    # Target-specific formats are derived from these.
    ESCAPE_BYTE_VALUES = {
      '<P>'  => 124,   # |
      '<R>'  => 93,    # ]
      '<L>'  => 91,    # [
    ...
    # Rust byte literal format
    RUST_ESCAPE_SEQUENCES = ESCAPE_BYTE_VALUES.transform_values do |byte|
      char = byte.chr
      escaped = case char
                when '\\' then '\\\\'
                when "'" then "\\'"
    ...
    # Ruby integer format (just the byte value as string)
    RUBY_ESCAPE_SEQUENCES = ESCAPE_BYTE_VALUES.transform_values(&:to_s)
                                              .merge('<>' => "''").freeze
    ...
    def rust_expr(str)
      ...
      result
        ...
        .gsub(/<[A-Z]+>/) { |m| RUST_ESCAPE_SEQUENCES[m] || m } # Escape sequences
    end
    ...
    # Transform function call arguments for Rust.
    # - :param -> param (parameter references)
    # - <R>, <RB>, etc. -> byte literals via RUST_ESCAPE_SEQUENCES
    # - Bare quotes: " -> b'"', ' -> b'\''
    def transform_call_args(args)
      args.split(',').map do |arg|
        arg = arg.strip
        # Try escape sequence lookup first
        if (escaped = RUST_ESCAPE_SEQUENCES[arg])
          escaped
        else
          case arg
    ...
    # Convert a character to Ruby byte value (Integer).
    # Examples: "\n" -> "10", "|" -> "124", " " -> "32"
    ...
    # Escape string for Ruby double-quoted string literal.
    ...
    # Convert to snake_case (for Ruby method/constant names).
    # Examples: "ElementStart" -> "element_start", "BoolTrue" -> "bool_true"
    def snakecase(str)
      return '' if str.nil?
    ...
    # Convert special class name to Ruby predicate method call.
    # Examples: "letter" -> "letter?", "xid_start" -> "xid_start?"
    ...
    # - /function(args) -> parse_function(transformed_args, &on_event)
    # - /function -> parse_function(&on_event)
    # - COL -> col
    # - LINE -> @line
    # - PREV -> prev
    # - Character literals: 'x' -> 120 (byte value)
    # - Escape sequences: <R> -> 93, <P> -> 124, etc.
    # - Parameter references: :param -> param
    # - Rust byte literals: b"x" -> "x", NNu8 -> NN (IR builder pre-transforms to Rust)
    def ruby_expr(str)
      return '' if str.nil?

      result = str.to_s

      # Convert Rust byte literals that IR builder pre-generated (workaround)
      # b"..." -> "..." (byte string to regular string)
      result = result.gsub(/b"([^"]*)"/) { "\"#{::Regexp.last_match(1)}\"" }
      # b'x' -> NN (byte char to integer)
      result = result.gsub(/b'([^']*)'/) do
        ...
        when '\\\\' then '92'
        when "\\'" then '39'
        else char.bytes.first.to_s
        end
      end
      # NNu8 -> NN (remove Rust type suffix)
      result = result.gsub(/(\d+)u8\b/, '\1')

      # Process function calls FIRST, before variable expansion
      result = result.gsub(%r{/(\w+)\(([^)]*)\)}) do
        func = ::Regexp.last_match(1)
        args = transform_ruby_call_args(::Regexp.last_match(2))
        args = expand_ruby_vars(args)
        "parse_#{func}(#{args}, &on_event)"
      end
      result = result.gsub(%r{/(\w+)}) { "parse_#{::Regexp.last_match(1)}(&on_event)" }

      # Expand special variables
      result = expand_ruby_vars(result)

      # Transform standalone args
      result = transform_ruby_call_args(result)

      # Convert char literals to byte values
      ...
      # Escape sequences (including <> for empty)
      result.gsub(/<[A-Z]*>/) { |m| RUBY_ESCAPE_SEQUENCES[m] || m }
    end

    # Expand special variables for Ruby.
    def expand_ruby_vars(str)
      str
        .gsub(/\bCOL\b/, 'col')
        .gsub(/\bLINE\b/, '@line')
        .gsub(/\bPREV\b/, 'prev')
    ...
    # Transform function call arguments for Ruby.
    def transform_ruby_call_args(args)
      args.split(',').map do |arg|
        arg = arg.strip
        if (escaped = RUBY_ESCAPE_SEQUENCES[arg])
          escaped
        else
          case arg
          when /^:(\w+)$/           then ::Regexp.last_match(1) # :param -> param
          when '"'                  then '34'  # Double quote byte value
          when "'"                  then '39'  # Single quote byte value
          when /^\d+$/              then arg   # numeric literals
          when /^-?\d+$/            then arg   # negative numbers
          when /^'(.)'$/            then ::Regexp.last_match(1).ord.to_s # char literal -> byte value
          when /^"(.)"$/            then ::Regexp.last_match(1).ord.to_s # quoted char
          when %r{^[!;:#*\-_<>/\\@$%^&+=?,.]$} then arg.ord.to_s # Single punctuation → byte value
          else arg # pass through (variables, expressions)
          end
        end
      end.join(', ')
  ...
  class Generator
    ...
    def collect_mutable_vars(commands, mutable)
      commands.each do |cmd|
        ...
        when :conditional                      then cmd.args[:clauses]&.each { |clause| collect_mutable_vars(clause.commands, mutable) }
        end
      end
    end
